...
class GCN(nn.Module):
    ...
    def forward(self, g, h):
        h = self.drop(h)
        h = self.proj(h) # 아래와 순서 바꿈
        h = torch.matmul(g, h) # 위와 순서 바꿈
        h = self.act(h)
        return h
...
class DualGCN(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, act, p):
        ...
        self.proj3 = nn.Linear(in_dim, out_dim)
        self.act = act
        self.drop = nn.Dropout(p=p) if p > 0.0 else nn.Identity()
...
#def modularity_rank(g, scores):
    ## function for removing high bridge top k-rank
    # g: graph, scores: list of nodes' score
